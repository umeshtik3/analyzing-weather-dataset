...
def numerical(df):
    ...
    """
    numerical_var = weather.select_dtypes(include='number').columns
...
def clear(df,col,val):
    ...
    """
    value_counts = (df[col]==val).value_counts()[1]
    return value_counts
...
def agg_values_ina_month(df,date_col,agg_col, agg):
    ...
    aggregated_value = df.pivot_table(values=agg_col,index=date_col,aggfunc={agg_col:aggregate[agg]})
...
def convert(df,celsius):
    ...
    """
    df['fahrenhheit'] = df[celsius].map(lambda x : ((x * (9/5))+32))
    converted_temp = df
